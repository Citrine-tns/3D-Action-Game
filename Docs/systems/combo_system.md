...
### 概要
コンボスロットに「フォロースルー」フラグがある場合、このノードのスイングが始まる前に、前ノードの終端位置から体の後ろを経由してこのノードの始動位置へ遷移する「回し」アニメーションが入る

### フラグの所在
- フォロースルーは**受け側のスロット**（次のノード）に定義する
- 同じノードでもプリセットによってフォロースルーあり/なしを使い分けられる
- 1段目のスロットにフォロースルーを設定しても意味はない（前ノードがないため）

### 効果
前ノードの終了位置を**中心対角**に反転させて接続判定を行う
...
### 挙動
- 前ノードの Recovery 中に入力があり、次スロットにフォロースルーフラグがある場合:
  1. 前ノードの終端位置から「回し」アニメーションが再生される
  2. 回しの間は攻撃判定なし・移動不可
  3. 回し完了後、このノードの始動位置に到達
  4. このノードの Active（実際のスイング）が始まる
